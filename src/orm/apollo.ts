...
export class Apollo {
  ...
  constructor(connection, options: any = {}) {
    if (!connection) {
      throw buildError(
        'model.validator.invalidconfig',
        'Cassandra connection configuration undefined',
      );
    }
    ...
    this._client = null;
    this._esClient = null;
    this._gremlinClient = null;
  }

  _generateModel(properties) {
  ...
  createEsClient() {
    if (!elasticsearch) {
      throw new Error(
        'Configured to use elassandra, but elasticsearch module was not found, try npm install elasticsearch',
      );
    }
  ...
  assertEsIndex(callback) {
    const esClient = this.createEsClient();
  ...
  createGremlinClient() {
    if (!gremlin) {
      throw new Error(
        'Configured to use janus graph server, but gremlin module was not found, try npm install gremlin',
      );
    }
    ...
    this._gremlinClient = gremlin.createClient(
      gremlinConfig.port,
      gremlinConfig.host,
      gremlinConfig.options,
    );
    this._gremlinClient = gremlinConfig;
    return this._gremlinClient;
  }

  _assertGremlinGraph(callback) {
    const gremlinClient = this.createGremlinClient();
  ...
  _assertKeyspace(callback) {
    ...
    keyspaceBuilder.get_keyspace(keyspaceName, (err, keyspaceObject) => {
      ...
      if (!keyspaceObject) {
        keyspaceBuilder.create_keyspace(
          keyspaceName,
          options.defaultReplicationStrategy,
          callback,
        );
      ...
      const dbReplication = normalizer.normalize_replication_option(
        keyspaceObject.replication,
      );
      const ormReplication = normalizer.normalize_replication_option(
        options.defaultReplicationStrategy,
      );

      if (!_.isEqual(dbReplication, ormReplication)) {
        keyspaceBuilder.alter_keyspace(
          keyspaceName,
          options.defaultReplicationStrategy,
          callback,
        );
  ...
  _assertUserDefinedTypes(callback) {
    const client = this._defineConnection;
    const options = this._options;
      ...
      (udtKey) =>
        new Promise((resolve, reject) => {
          ...
          udtBuilder.get_udt(udtKey, keyspace, (err, udtObject) => {
            ...
            const udtValues = _.map(
              _.values(options.udts[udtKey]),
              normalizer.normalize_user_defined_type,
            );
            const fieldNames = udtObject.field_names;
            const fieldTypes = _.map(
              udtObject.field_types,
              normalizer.normalize_user_defined_type,
            );

            if (
              _.difference(udtKeys, fieldNames).length === 0 &&
              _.difference(udtValues, fieldTypes).length === 0
            ) {
              udtCallback();
              return;
            }
  ...
  _assertUserDefinedFunctions(callback) {
    const client = this._defineConnection;
    const options = this._options;
    const keyspace = this._keyspace;
      ...
      (udfKey) =>
        new Promise((resolve, reject) => {
          ...
          udfBuilder.get_udf(udfKey, keyspace, (err, udfObject) => {
            ...
            const udfReturnType = normalizer.normalize_user_defined_type(
              options.udfs[udfKey].returnType,
            );
            const resultReturnType = normalizer.normalize_user_defined_type(
              udfObject.return_type,
            );

            const udfInputs = options.udfs[udfKey].inputs
              ? options.udfs[udfKey].inputs
              : {};
            const udfInputKeys = Object.keys(udfInputs);
            const udfInputValues = _.map(
              _.values(udfInputs),
              normalizer.normalize_user_defined_type,
            );
            const resultArgumentNames = udfObject.argument_names;
            const resultArgumentTypes = _.map(
              udfObject.argument_types,
              normalizer.normalize_user_defined_type,
            );

            if (
              udfLanguage === resultLanguage &&
              udfCode === resultCode &&
  ...
  _assertUserDefinedAggregates(callback) {
    const client = this._defineConnection;
    const options = this._options;
    const keyspace = this._keyspace;
      ...
      (udaKey) =>
        new Promise((resolve, reject) => {
          ...
          udaBuilder.get_uda(udaKey, keyspace, (err, udaObjects) => {
            ...
            const inputTypes = _.map(
              options.udas[udaKey].input_types,
              normalizer.normalize_user_defined_type,
            );
            const sfunc = options.udas[udaKey].sfunc.toLowerCase();
            const stype = normalizer.normalize_user_defined_type(
              options.udas[udaKey].stype,
            );
            const finalfunc = options.udas[udaKey].finalfunc
              ? options.udas[udaKey].finalfunc.toLowerCase()
              : null;
            const initcond = options.udas[udaKey].initcond
              ? options.udas[udaKey].initcond.replace(/[\s]/g, '')
              : null;

            for (let i = 0; i < udaObjects.length; i++) {
              const resultArgumentTypes = _.map(
                udaObjects[i].argument_types,
                normalizer.normalize_user_defined_type,
              );

              const resultStateFunc = udaObjects[i].state_func;
              const resultStateType = normalizer.normalize_user_defined_type(
                udaObjects[i].state_type,
              );
              const resultFinalFunc = udaObjects[i].final_func;
              const resultInitcond = udaObjects[i].initcond
                ? udaObjects[i].initcond.replace(/[\s]/g, '')
                : null;

              if (
                sfunc === resultStateFunc &&
  ...
  init(callback) {
    const onUserDefinedAggregates = (err) => {
      ...
      if (this._keyspace && this._options.manageESIndex) {
        this.assertESIndexAsync = Promise.promisify(this._assertEsIndex);
        managementTasks.push(this.assertESIndexAsync());
      }
      if (this._keyspace && this._options.manageGraphs) {
        this.assertGremlinGraphAsync = Promise.promisify(
          this._assertGremlinGraph,
        );
    ...
    const onUserDefinedFunctions = function f(err) {
      ...
      try {
        this._assert_user_defined_aggregates(
          onUserDefinedAggregates.bind(this),
        );
      } catch (e) {
    ...
    if (this._keyspace && this._options.createKeyspace !== false) {
      this._assertKeyspace(onKeyspace.bind(this));
    } else {
  ...
  addModel(modelName, modelSchema) {
    ...
    const baseProperties = {
      ...
      define_connection: this._defineConnection,
      cql: this._client,
      esclient: this._esClient,
      gremlin_client: this._gremlinClient,
    ...
    this._models[modelName] = this._generateModel(baseProperties);
  ...
  close(callback) {
    ...
    if (
      this.orm._gremlin_client &&
      this.orm._gremlin_client.connection &&
      this.orm._gremlin_client.connection.ws
    ) {
