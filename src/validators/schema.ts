...
import { TYPE_MAP } from './datatypes';
...
export class Schemer {
  ...
  static hasField(modelSchema, fieldName) {
  ...
  static validateField(modelSchema, fieldObject, fieldName) {
    if (!fieldObject) {
      throw (new Error(util.format('Schema field "%s" is not properly defined', fieldName)));
    }
    const fieldType = Schemer.getFieldType(modelSchema, fieldName);
    if (!_.has(TYPE_MAP, fieldType)) {
    ...
    if (!(Schemer.isFieldDefaultValueValid(modelSchema, fieldName))) {
      throw (new Error(util.format('Invalid default value for field: %s(%s)', fieldName, fieldType)));
    }
  }

  static validatePrimaryKey(modelSchema) {
    if (typeof (modelSchema.key[0]) === 'string') {
      if (!Schemer.hasField(modelSchema, modelSchema.key[0])) {
      ...
      modelSchema.key[0].forEach((partitionKeyField) => {
        if ((typeof (partitionKeyField) !== 'string') || !Schemer.hasField(modelSchema, partitionKeyField)) {
    ...
    modelSchema.key.forEach((primaryKeyField, primaryKeyIndex) => {
      if (primaryKeyIndex > 0) {
        if ((typeof (primaryKeyField) !== 'string') || !this.hasField(modelSchema, primaryKeyField)) {
  ...
  static validateMaterializedView(modelSchema, materializedViewObject, materializedViewName) {
    ...
    materializedViewObject.select.forEach((materializedViewSelectField) => {
      if ((typeof (materializedViewSelectField) !== 'string')
            || !(Schemer.hasField(modelSchema, materializedViewSelectField)
    ...
    if (typeof (materializedViewObject.key[0]) === 'string') {
      if (!this.hasField(modelSchema, materializedViewObject.key[0])) {
      ...
      materializedViewObject.key[0].forEach((materializedViewPartitionKeyField) => {
        if ((typeof (materializedViewPartitionKeyField) !== 'string')
            || !this.hasField(modelSchema, materializedViewPartitionKeyField)) {
          throw (new Error(util.format(
    ...
    materializedViewObject.key.forEach((materializedViewPrimaryKeyField, materializedViewPrimaryKeyIndex) => {
      if (materializedViewPrimaryKeyIndex > 0) {
        if ((typeof (materializedViewPrimaryKeyField) !== 'string')
            || !this.hasField(modelSchema, materializedViewPrimaryKeyField)) {
  ...
  static validateIndex(modelSchema, indexDef) {
    ...
    if (indexNameList.length > 1) {
      ...
      if (!this.hasField(modelSchema, indexNameList[1])) {
      ...
      if (!this.hasField(modelSchema, indexNameList[0])) {
  ...
  static validateCustomIndex(modelSchema, customIndex) {
    if (!_.isPlainObject(customIndex)) {
      throw (new Error('custom_index must be an object with proper indexing attributes'));
    }
    if ((typeof (customIndex.on) !== 'string') || !this.hasField(modelSchema, customIndex.on)) {
  ...
  static validateModelSchema(modelSchema) {
    if (!modelSchema) {
    ...
    _.forEach(modelSchema.fields, (fieldObject, fieldName) => {
      this.validateField(modelSchema, fieldObject, fieldName);
    });

    this.validatePrimaryKey(modelSchema);
    ...
    if (modelSchema.materialized_views) {
      ...
      _.forEach(modelSchema.materialized_views, (materializedViewObject, materializedViewName) => {
        this.validateMaterializedView(modelSchema, materializedViewObject, materializedViewName);
      });
    }
    ...
    if (modelSchema.indexes) {
      ...
      modelSchema.indexes.forEach((indexDef) => {
        this.validateIndex(modelSchema, indexDef);
      });
    }
    ...
    if (modelSchema.custom_index) {
      this.validateCustomIndex(modelSchema, modelSchema.custom_index);
    }
    ...
    if (modelSchema.custom_indexes) {
      ...
      modelSchema.custom_indexes.forEach((customIndex) => {
        this.validateCustomIndex(modelSchema, customIndex);
      });
    }
  }

  static formatValidationRule(rule, fieldname) {
    ...
    if (!rule.message) {
      rule.message = this.getGenericValidationMessage;
    }
  ...
  static getGenericValidationMessage(value, propName, fieldtype) {
    return util.format('Invalid Value: "%s" for Field: %s (Type: %s)', value, propName, fieldtype);
  }

  static getValidationMessage(validators, value) {
  ...
  static getValidators(modelSchema, fieldName) {
    const validators = [];
    const fieldType = Schemer.getFieldType(modelSchema, fieldName);
    const typeFieldValidator = TYPE_MAP.genericTypeValidator(fieldType);
    ...
    if (typeof field.rule !== 'undefined') {
      if (typeof field.rule === 'function') {
        field.rule = {
          ...
          message: this.getGenericValidationMessage,
        };
        validators.push(field.rule);
      } else if (Array.isArray(field.rule.validators)) {
        field.rule.validators.forEach((fieldRule) => {
          validators.push(this.formatValidationRule(fieldRule, fieldName));
        });
      } else if (field.rule.validator) {
        validators.push(this.formatValidationRule(field.rule, fieldName));
      }
    }
  ...
  static getFieldType(modelSchema, fieldName) {
  ...
  static isRequiredField(modelSchema, fieldName) {
    return !!(modelSchema.fields[fieldName].rule && modelSchema.fields[fieldName].rule.required);
  }

  static isPrimaryKeyField(modelSchema, fieldName) {
    if (modelSchema.key.includes(fieldName)) {
      return true;
    }
    return !!(modelSchema.key[0] instanceof Array && modelSchema.key[0].includes(fieldName));
  }

  static isFieldDefaultValueValid(modelSchema, fieldName) {
